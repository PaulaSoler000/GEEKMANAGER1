...
if (!empty($buscar)) {
    ...
    $filtro = "%{$buscar}%";
    $sql->execute([$user_id, "manga", $filtro, $filtro, $empezar_desde, $registros_por_pagina]);
} else {
    if (!is_null($filt_tag)) {
        $sql = $conexion->prepare($sql_filt);
        $filtro = "%{$filt_tag}%";
        $sql->execute([$user_id, "manga", $filtro, $empezar_desde, $registros_por_pagina]);
    } else {
        $sql = $conexion->prepare($sql_no_filt);
        $sql->execute([$user_id, "manga", $empezar_desde, $registros_por_pagina]);
    }
}
?>
...
<body>
    ...
    <nav>
        ...
        <ul class="nav-links">
            ...
            <li class="centro active"><a href="mangas.php">Mangas</a></li>
