...
if (!empty($buscar)) {
    ...
    $filtro = "%{$buscar}%";
    $sql->execute([$user_id, "videojuego", $filtro, $filtro, $empezar_desde, $registros_por_pagina]);
} else {
    if (!is_null($filt_tag)) {
        $sql = $conexion->prepare($sql_filt);
        $filtro = "%{$filt_tag}%";
        $sql->execute([$user_id, "videojuego", $filtro, $empezar_desde, $registros_por_pagina]);
    } else {
        $sql = $conexion->prepare($sql_no_filt);
        $sql->execute([$user_id, "videojuego", $empezar_desde, $registros_por_pagina]);
    }
}
